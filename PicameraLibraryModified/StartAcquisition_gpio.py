#Any Changes made from the original author are followed by a #ZC and what change was made
...
import sys, getopt
import argparse
#import RPi.GPIO as GPIO #ZC Commented out (TTL and Arduino related)
import os
...
#BOUNCETIME=10 #ZC Commented out because ^
camId = str(5) #ZC manually changed this value on each raspberry pi
...
#TTL_FILE_NAME = "cam"+ camId + "_ttl_" + str(dt.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")) + ".csv" #ZC Commented out, no such file needed
...
#ZC Commented out all of above ^, All relating to TTL programming       
...
class TimestampOutput(object):
    def __init__(self, camera, video_filename, timestamp_filename): #ttl_filename
        ...
        #self._ttlFile = ttl_filename #ZC Commented out, no such file being created
        self._timestamps = []
        #self._ttlTimestamps = [] #ZC Commented out, none of this type to record in the list.
     ...
     #   inputState = GPIO.input(input_pin)
      #  GPIO.remove_event_detect(input_pin)
       # if self.camera.frame.timestamp is not None:
        ...
        #GPIO.add_event_detect(pinTTL, GPIO.BOTH, bouncetime=BOUNCETIME)
#ZC Commented out, no such file needs to be created
...
with PiCamera(resolution=(WIDTH, HEIGHT), framerate=FRAMERATE) as camera:
    ...
    try:
        ...
        #GPIO.cleanup() #ZC commented out GPIO not imported or needed.
